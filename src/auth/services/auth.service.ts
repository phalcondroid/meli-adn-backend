...
export class AuthService {
  ...
  async validateUser(username: string, pass: string): Promise<UserJwt> {
    const user: UsersModel = await this.usersManager.getUserByUsername(username);
    if (!user) {
      return;
    }
  ...
  public async login(user: UserJwt): Promise<StartSessionResponse> {
    ...
    const userInfo: UsersModel = await this.usersManager.getUserByUsername(user.username);
